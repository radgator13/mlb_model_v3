...
OUTPUT_CSV = 'mlb_fielding_stats_2025.csv'
PENDING_LOG = 'pending_fielding_boxscores.csv'
...
def parse_fielding_stats(game, boxscore):
    game_pk = game['gamePk']
    ...
    for side in ['home', 'away']:
        ...
        for player_key, player_data in players.items():
            ...
            rows.append({
                ...
                'put_outs': stats.get('putOuts'),
                'errors': stats.get('errors'),
                'chances': stats.get('chances'),
                'fielding': stats.get('fielding'),
                'range_factor': stats.get('rangeFactor'),
                'zone_rating': stats.get('zoneRating')
            })
...
def run_fielding_pipeline(start_date='2025-03-27'):
    ...
    for n in range((today - datetime.date.fromisoformat(start_date)).days + 1):
        ...
        for game in games:
            if game['status']['detailedState'] == 'Final':
                ...
                if boxscore:
                    parsed_fielding = parse_fielding_stats(game, boxscore)
                    new_data.extend(parsed_fielding)
                    time.sleep(0.5)
                else:
    ...
    if new_data:
        ...
        print(f"✅ Added {len(new_data)} new fielding lines.")
    else:
        print("No new completed games to add.")

if __name__ == "__main__":
    run_fielding_pipeline()
